...
<body>
	...
	<div class="g_6 contents_options">
		<div class="simple_buttons" id="valid">
			<div class="bwIcon i_16_settings">
				<form action="${pageContext.request.contextPath}/document/auditTempDoc/${tempDoc.get(0).getId()}" method="post">
					<input type="hidden" name="_method" value="put" />
					<input type="submit" name="state" value="通过" class="btn"/>
					<input type="submit" name="state" value="不通过" class="btn"/>
			    </form>
			</div>
		</div>
	</div>
	...
	<%-- <form action="${pageContext.request.contextPath}/document/auditTempDoc/${tempDoc.get(0).getId()}" method="post">
		<input type="hidden" name="_method" value="put" />
		<input type="submit" name="state" value="通过" />
		<input type="submit" name="state" value="不通过" />
    </form>
	<h5>
		文件类型：
		<c:if test="${tempDoc.get(0).getType() eq 'image' }">图片</c:if>
