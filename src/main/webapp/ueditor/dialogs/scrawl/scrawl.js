...
function exec(scrawlObj) {
    if (scrawlObj.isScrawl) {
        ...
        if (!!base64) {
                ...
                onsuccess:function (xhr) {
                    if (!scrawlObj.isCancelScrawl) {
                        ...
                        if (responseObj.state == "SUCCESS") {
                            var imgObj = {},
                                url = editor.options.contextRootPath + editor.options.imageUrl + responseObj.url;
                            imgObj.src = url;
                            imgObj._src = url;
            ...
            var basePath = editor.getOpt("contextRootPath");
            var actionUrl = basePath + editor.getOpt('scrawlActionName'),
                params = '&filetype=scrawl';
                url = utils.formatUrl(actionUrl + (actionUrl.indexOf('?') == -1 ? '?':'&') + params);
                console.log("scrawl.url:"+url);
